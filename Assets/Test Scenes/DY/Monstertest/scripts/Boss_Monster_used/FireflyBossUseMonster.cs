...
public class FireflyBossUseMonster : Character
{
    [Header("Prameter")]
    ...
    */
    ...
    public void DirectionFireflyBossUsemonster(float target, float baseobj)
    {
        if (target < baseobj)
            anim.SetFloat("Direction", -1);
        else
    ...
    public void FireflyBossUseAttack() //인스턴시에이트 써서 총알 나갈  수 있도록 만들기
    {
